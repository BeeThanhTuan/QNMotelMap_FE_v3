...
export class MotelsComponent {
  ...
  currentIndexMotel = 0;
  constructor(private motelService: MotelService, private alert: AlertService){}
  ...
  getAllMotels():void{
    this.motelService.getAllMotels().pipe(
  ...
  showSoftDeleteMotelAlert(title: string, message: string, id: string, index: number) {
    Swal.fire({
      title: title,
      text: message,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.motelService.softDeleteMotelByID(id).subscribe({
            next: (response) => {
              ...
              this.alert.showSuccess('Xoá thành công!', 'Bạn đã xoá thành công nhà trọ.')
            },
            error: (roleError) => {
  ...
  nextImage(index:number): void {
    this.currentIndexMotel = (this.currentIndexMotel + 1) % this.listMotels[index].ListImages.length;
  }

  prevImage(index:number): void {
    this.currentIndexMotel = (this.currentIndexMotel - 1 + this.listMotels[index].ListImages.length) % this.listMotels[index].ListImages.length;
  }
